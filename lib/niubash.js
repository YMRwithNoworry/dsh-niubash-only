...
export const niuRuntime = {
  ...
  /**
   * Whether this deployment hands shell commands to `ctx.sandbox`. `false`
   * (the bundle default) means `niu` runs directly on the host; the teaching
   * layer reads this fact so the tool description never promises sandbox
   * markers that cannot appear.
   */
  sandbox: undefined,
}
