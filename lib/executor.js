...
export class NiubashExecutor extends BaseExecutor {
   ...
   * of the Windows family (`confine(spec, policy)`): wrap the Niubash argv
  ...
  confine(subject, policy) {
    ...
    return this.ctx.sandbox.confine(this.niuShellArgv(command), policy)
  }

  /**
   * Foreground run. The confined path is the parent's (which now wraps the
   * Niubash argv through the `confine` override); the unconfined path is rebuilt
   * here so `danger-full-access` does not fall back to the base shell's argv.
   * @param spec - a resolved spec from {@link resolve}.
   * @returns the settled result, with sandbox facts as the parent reports them.
   */
  async run(spec) {
    if (!isUnconfined(this, spec)) return this.withDialectHint(spec, await super.run(spec))
    const result = await this.runArgv(spec, this.niuShellArgv(spec.command))
    return this.withDialectHint(spec, { ...result, sandbox: { mode: 'danger-full-access', denied: false } })
  }
  ...
  start(spec) {
    ...
    return this.startArgv(spec, this.niuShellArgv(spec.command))
  }
